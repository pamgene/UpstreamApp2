...
shinyServerRun = function(input, output, session, context) {
  ...
  observe({
    ...
    output$status = renderText({
      ...
      settings = data.frame(setting = c("Kinase family", "ScanRank Min", "ScanRank Max", "Number of Permutations", "In Vitro In Vitro weight", "PhosphoNET weight", "Min PhosphoNet score", "Min Sequence Similarity"),
...
shinyServerShowResults = function(input, output, session, context){
  ...
  observe({
    ...
    scorePlot = reactive({
      ...
      kinLabelGroup <- aSub %>% distinct(ClassName, Kinase_group) %>% arrange(match(ClassName, axis_order)) %>% pull(Kinase_group)
